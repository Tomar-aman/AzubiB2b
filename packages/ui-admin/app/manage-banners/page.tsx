"use client";
import MainLayout from "@/components/layout";
import React, { useEffect, useState } from "react";
import {
  Box,
  Button,
  CircularProgress,
  Dialog,
  DialogActions,
  DialogContent,
  Paper,
  Table,
  TableBody,
  TableCell,
  TableContainer,
  TableHead,
  TableRow,
  Typography,
} from "@mui/material";
import TextInput from "@/components/labelInput";
import { SVG } from "@/assets/svg";
// import CustomPagination from "@/components/pagination";
import Link from "next/link";
import BannerStyled from "./bannerStyled";
import { BannerApi } from "../api/banner/BannerApi";

interface Banner {
  _id: string;
  bannerTitle: string;
  companyName: string;
  job: string;
  jobUrl: string;
}

function ManageBanners() {
  const [loading, setLoading] = useState(false);
  const [banners, setBanners] = useState<Banner[]>([]);
  const [recordPerPage, setRecordPerPage] = useState(10);
  const [searchValue, setSearchValue] = useState("");
  const [open, setOpen] = useState(false);
  const [selectedBannerId, setSelectedBannerId] = useState("");

  const handleBanners = async (recordPerPage: number, searchValue: string) => {
    setLoading(true);
    const response: any = await BannerApi.getAllBanners(
      recordPerPage,
      searchValue,
    );
 
    if (response?.remote === "success") {
      setBanners(response.data.data);
    } else {
      return response?.error;
    }
    setLoading(false);
  };

  useEffect(() => {
    handleBanners(recordPerPage, searchValue);
  }, [recordPerPage, searchValue]);

  const handleSearchChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    setSearchValue(event.target.value); // Update search value on input change
  };

  const handleClickOpen = (id: any) => {
    setSelectedBannerId(id);
    setOpen(true);
  };

  const handleClose = () => {
    setOpen(false);
  };

  const handleDelete = async (id: string) => {
    setLoading(true);
    const response = await BannerApi.deleteBanner(id);
    if (response.remote === "success") {
      setBanners((prevBanners) =>
        prevBanners.filter((banner) => banner._id !== id),
      );
    } else {
      alert("Failed to delete the banner. Please try again.");
    }
    setOpen(false);
    setLoading(false);
  };

  return (
    <BannerStyled>
      <MainLayout>
        <Box className="dashboardContent">
          <Box sx={{ p: 4 }}>
            <Typography variant="h5" sx={{ mb: 2 }} className="title">
              Manage banners
            </Typography>
            <Box className="searchTag">
              <SVG.Search className="images" />
              <TextInput
                placeholder="Search"
                className="searchBar"
                value={searchValue}
                onChange={handleSearchChange}
              />
              <Link href="/manage-banners/add-banner">
                {" "}
                <Box className="addNew">
                  <SVG.Add />
                  <span>ADD NEW</span>
                </Box>
              </Link>
            </Box>

            {/* Table */}
            {loading ? (
              <CircularProgress />
            ) : (
              <TableContainer component={Paper} className="mainTable">
                <Table>
                  <TableHead>
                    <TableRow className="tableHead">
                      <TableCell width="20%">Action</TableCell>
                      <TableCell width="20%">Title</TableCell>
                      <TableCell width="20%">Company name</TableCell>
                      <TableCell width="20%">Job</TableCell>
                      <TableCell width="20%">Job url</TableCell>
                    </TableRow>
                  </TableHead>
                  <TableBody className="tbody">
                    {banners && banners.length > 0 ? (
                      banners.map((banner: Banner, index: number) => (
                        <TableRow key={index}>
                          <TableCell>
                            <Box className="settingTool">
                              <Link
                                href={`/manage-banners/add-banner?id=${banner._id}`}
                              >
                                {" "}
                                <SVG.Pencil />
                              </Link>
                              <Link
                                className="btnLink"
                                onClick={() => handleClickOpen(banner._id)}
                                href={""}
                              >
                                <SVG.Trash
                                  onClick={() => handleClickOpen(banner._id)}
                                />
                              </Link>
                              <Dialog
                                open={open}
                                className="deleteDialog"
                                onClose={handleClose}
                              >
                                <DialogContent>
                                  <Typography
                                    style={{
                                      fontWeight: 500,
                                      fontSize: "30px",
                                      lineHeight: "32px",
                                      fontFamily: "poppins",
                                      textAlign: "center",
                                      margin: "24px",
                                    }}
                                  >
                                    Are you sure you want to delete this banner?
                                  </Typography>
                                </DialogContent>
                                <DialogActions className="footerButton">
                                  <Button onClick={handleClose} color="primary">
                                    Cancel
                                  </Button>
                                  <Button
                                    onClick={() =>
                                      handleDelete(selectedBannerId)
                                    }
                                    color="error"
                                  >
                                    Remove
                                  </Button>
                                </DialogActions>
                              </Dialog>
                            </Box>
                          </TableCell>
                          <TableCell width="20%">
                            {banner.bannerTitle ? banner.bannerTitle : "-"}
                          </TableCell>
                          <TableCell width="20%">
                            {banner.companyName ? banner.companyName : "-"}
                          </TableCell>
                          <TableCell width="20%">
                            {banner.job ? banner.job : "-"}
                          </TableCell>
                          <TableCell width="20%">
                            {banner.jobUrl ? banner.jobUrl : "-"}
                          </TableCell>
                        </TableRow>
                      ))
                    ) : (
                      <TableRow>
                        <TableCell colSpan={5} align="center">
                          No companies found
                        </TableCell>
                      </TableRow>
                    )}
                  </TableBody>
                </Table>
              </TableContainer>
            )}
            {/* Pagination */}
            {/* <CustomPagination /> */}
          </Box>
        </Box>
      </MainLayout>
    </BannerStyled>
  );
}
export default ManageBanners;
